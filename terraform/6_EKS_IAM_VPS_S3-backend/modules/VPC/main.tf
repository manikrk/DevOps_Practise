...
resource "aws_internet_gateway" "vrp_vpc_igw" {
    vpc_id = aws_vpc.vrp_vpc.id
...
resource "aws_route_table" "route_table_vrp_vpc" {
    vpc_id = aws_vpc.vrp_vpc.id

    route {
        cidr_block = "0.0.0.0/0"
        gateway_id = aws_internet_gateway.vrp_vpc_igw.id
    }
...
resource "aws_route_table_association" "RT_association" {
  ...
  route_table_id = aws_route_table.route_table_vrp_vpc.id
}
